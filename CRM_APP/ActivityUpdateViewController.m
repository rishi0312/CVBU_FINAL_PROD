    ...
    if (![self connected])
    {
        ...
        if(flagset==1)
        {
            ...
            self.txtActivityStatus.text = opportunity_list.LAST_PENDING_ACTIVITY_STATUS;//Abhi
             ...
             NSLog(@"activityPending_list.ACTIVITY_DESC......%@",activityPending_list.ACTIVITY_DESC);
            ...
            self.lblTGMTKMNumber.text=activity_list.TGM_TKM_PHONE_NUMBER;
            self.lblAccountType.text=activity_list.ACCOUNT_TYPE;
            self.lblOptyId.text=activity_list.OPTY_ID;
          ...
          self.txtActivityStatus.text=activityPending_list.ACTIVITY_STATUS;
             self.txtfldTym.text=activityPending_list.ACTIVITY_PLAN_START_TM;
...
- (IBAction)NextPlanDateAction:(id)sender {
    ...
    NSString* stringDate_ = [formatter stringFromDate:date];
    
    NSLog(@"Date..Set  From : : %@",stringDate_);
    
    self.txtNextPlanDate.text = stringDate_;
...
-(void)ActivityType_Found:(NSNotification*)notification
{
    ...
    else if ([response rangeOfString:@"SOAP:Fault"].location != NSNotFound )
    {
        ...
        if (tuple2)
        {
            ...
            [activityTypeArr addObject:@"Follow-Up"];
...
- (IBAction)btnDate:(id)sender {
    ...
    //[formatter setDateFormat:@"MM/dd/yyyy"]; //Abhishek Comment
    [formatter setDateFormat:@"dd-MMM-yyyy"];
...
- (IBAction)btnUpdate:(id)sender
{
    ...
    if (![self connected]) {
        ...
        if ([textfieldType.text isEqualToString:@"Follow-Up"] &&[txtActivityStatus.text isEqualToString:@"Done"]&&
            ([txtNextPlanDate.text isEqualToString:@""] ||[txtNextPlanTime.text isEqualToString:@""]) )  {
            
            NSLog(@"\n Test....Next plan Date and next Plan Type is mandetory ");
            alert = [[UIAlertView alloc] initWithTitle:@"DSE" message:@"Please enter mandatory fields for the activity Type" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
            [alert show];
        ...
        NSLog(@"\n self.txtfldTym.text..... %@",self.txtfldTym.text);
        
        NSLog(@"\n txtfldDate.text....... %@",self.txtfldDate.text);
        
        
        NSString *txtfldDate_ = self.txtfldDate.text;
        
        NSDateFormatter* formatter = [[NSDateFormatter alloc] init];
        [formatter setDateFormat:@"dd-MMM-yyyy"];
        //[formatter setDateFormat:@"MM/dd/yyyy"];
        NSDate* aDate_ = [formatter dateFromString:txtfldDate_];
        [formatter setDateFormat:@"MM/dd/yyyy"];
        NSLog(@"%@", [formatter stringFromDate:aDate_]);
        stringDate = [formatter stringFromDate:aDate_];
        NSLog(@"New Date : %@",stringDate);
...
-(void)NewCallActivityUpdate{
    ...
    [formatter setDateFormat:@"dd-MMM-yyyy"];
    //[formatter setDateFormat:@"MM/dd/yyyy"];
    NSDate* aDate_ = [formatter dateFromString:NewPlanDate];
    [formatter setDateFormat:@"MM/dd/yyyy"];
    NSLog(@"%@", [formatter stringFromDate:aDate_]);
    NewPlanDate = [formatter stringFromDate:aDate_];
    NSLog(@"New Date : %@",NewPlanDate);
    ...
    if ([textfieldType.text isEqualToString:@"Follow-Up"] &&[txtActivityStatus.text isEqualToString:@"Done"] ) {
        ...
        envelopeText = [NSString stringWithFormat:
                        ...
                        @"</SOAP:Body>"
                        @"</SOAP:Envelope>",activityPending_list.ACTIVITY_ID,textfieldType.text,stringDate,txDescription.text,newPlanDateTime ];
...
-(void)ActivityUpdatesNEW_Found:(NSNotification*)notification
{
    ...
    else if ([response rangeOfString:@"SOAP:Fault"].location != NSNotFound )
    {
        ...
        alert=[[UIAlertView alloc]initWithTitle:@"Alert" message :@"Something went wrong.Please try again" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        [alert show];
        ...
        if ([textfieldType.text isEqualToString:@"Follow-Up"] &&[txtActivityStatus.text isEqualToString:@"Done"] ) {
            ...
            [self hideAlert];
            UIAlertView *alertView=[[UIAlertView alloc]initWithTitle:nil message :@"Activity Updated Successfully" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
            [alertView show];
...
-(void)CallActivityUpdate
{
    ...
    if([userDetailsVal_.POSITION_TYPE isEqual:@"NDRM"]){
        ...
        envelopeText = [NSString stringWithFormat:
                        ...
                        @"</asi:SFATMCVContactInsertOrUpdate_Input>"
                        @"</soapenv:Body>"
                        @"</soapenv:Envelope>",opportunity_list.CONTACT_ID,opportunity_list.OPTY_ID,IntegrationIdvalue,IntegrationIdvalue,opportunity_list.OPTY_ID,stringDate];
    }
    else{
        ...
        envelopeText = [NSString stringWithFormat:
                        ...
                        @"<ListOfOpportunityRelatedActivities>"
                        @"<OpportunityRelatedActivities>"
                        @"<ActivityUID>%@</ActivityUID>" //pass integration ID
                        @"<TMIntegrationId>%@</TMIntegrationId>" //pass integration ID
                        @"<OpportunityId>%@</OpportunityId>"
                        @"<ActivityStatus>%@</ActivityStatus>"
                        @"<NextPlannedDate/>"
                        @"<Mode>Visit</Mode>"
                        ...
                        @"</asi:SFATMCVContactInsertOrUpdate_Input>"
                        @"</soapenv:Body>"
                        @"</soapenv:Envelope>",opportunity_list.CONTACT_ID,opportunity_list.OPTY_ID,IntegrationIdvalue,IntegrationIdvalue,opportunity_list.OPTY_ID,ActivityStatus,textfieldType.text,newPlanDateTime,txDescription.text,userDetailsVal_.PRIMARY_EMP,userDetailsVal_.Login_Name,userDetailsVal_.PRIMARY_EMP];
