...
//                    self.lbl_ActivityCount.text = PPL_ID_;
                    ...
                    TBXMLElement* MISSED = [TBXML childElementNamed:@"PENDING_ACTIVITYS" parentElement:table];
...
- (void)SalesPipeLineDashboard_DSE_Found:(NSNotification*)notification
{
    ...
    if ([response isEqual:@""]) {
        ...
        if (tuple) {
            ...
            [self hideAlert];
            alert = [[UIAlertView alloc] initWithTitle:nil message:@"Something went wronggggg.Please try again" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
            [alert show];
        }
    }
}
