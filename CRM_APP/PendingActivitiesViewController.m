...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    if (tableView == self.tableView) {
        ...
        if ([activityPending_list.ACTIVITY_STATUS isEqualToString:@"Done"]) {
            ...
            [cell.activityUpdateBtn setBackgroundColor:[UIColor colorWithRed:(79.0/255.0) green:(148.0/255.0) blue:(240.0/255.0) alpha:1.0]];
            [cell.activittyDonebtn setBackgroundColor:[UIColor colorWithRed:(79.0/255.0) green:(148.0/255.0) blue:(240.0/255.0) alpha:1.0]];
                ...
                if (tuple) {
                    do {
                        ...
                        if([activityPending_list.ACTIVITY_PLAN_START_DT isEqualToString:@""])
                        {
                            ...
                            activityPending_list.ACTIVITY_PLAN_START_TM = lastString;
                        }//ACTIVITY_TYPE
...
- (void)callPendingActivityList
{
    ...
    NSArray* foo = [activity_list.PLANNED_START_DATE componentsSeparatedByString: @" "];
activityPending_list.ACTIVITY_PLAN_START_DT= [foo objectAtIndex: 0];
activityPending_list.ACTIVITY_PLAN_START_TM= [foo objectAtIndex: 1];
    

    
   
   // activityPending_list.ACTIVITY_PLAN_START_DT=activity_list.PLANNED_START_DATE;
    activityPending_list.ACTIVITY_TYPE=activity_list.ACTIVITY_PENDING_TYPE;
    
    activityPending_list.OPTY_ID=activity_list.OPTY_ID;
    
    NSLog(@"opty id value%@",activityPending_list.OPTY_ID);
    
    
    
    activityPending_list.ACTIVITY_STATUS=activity_list.ACTIVITY_STATUS;
    
    
    NSLog(@"activity descr %@",activity_list.ACTIVITY_DESCRIPTION);
    
    
    activityPending_list.ACTIVITY_DESC=activity_list.ACTIVITY_DESCRIPTION;
   // activityPending_list.ACTIVITY_PLAN_START_TM=activity_list.PLANNED_START_DATE;
 
     [pendingActivities_ListArr addObject:activityPending_list];
    
    
    NSLog(@"pending activitieslistArray %@,%@",activityPending_list.ACTIVITY_PLAN_START_DT,activityPending_list.ACTIVITY_PLAN_START_TM);
